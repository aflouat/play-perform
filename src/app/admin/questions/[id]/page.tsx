...
function Field({ label, value, onChange, textarea = false }: {
  ...
  return (
    <div>
      ...
      {textarea ? <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={2} className={cls} />
...
export default function EditQuestionPage({ params }: { params: Params }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    else setMsg('❌ Erreur. Vérifiez que vous êtes connecté en tant qu\'admin.');
  }
  ...
  return (
    <form onSubmit={handleSave} className="space-y-5 max-w-2xl">
      ...
      <Field label="Question" value={q.question} onChange={(v) => set('question', v)} textarea />
        ...
        <div className="flex gap-3">
          {['A','B','C','D'].map((opt) => (
            <label key={opt} className="flex items-center gap-1.5 cursor-pointer">
              <input type="radio" name="correct" value={opt} checked={q.correct_option_id === opt} onChange={() => set('correct_option_id', opt)} />
