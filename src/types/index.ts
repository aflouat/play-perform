...
export interface EngagementPing {
  userId: string;
  contentId: string;
  timestamp: Date;
...
export interface BrevetTopic {
  id: string;
  subject: Subject;
  name: string;
  gradeLevel: '5e' | '3e';
  masteryPercent: number;
  requiredForBrevet: boolean;
}
...
export interface XpGain {
  amount: number;
  reason: 'quiz-correct' | 'quiz-perfect' | 'time-spent' | 'daily-login' | 'badge-earned';
  timestamp: Date;
}
...
export const DIFFICULTY_META: Record<QuizDifficulty, {
  label: string;
  color: string;
  icon: string;
  xpBase: number;
...
export interface QuizQuestion {
  ...
  category?: string;
  question: string;
  questionAssisted?: string;
  emoji?: string;
  imageUrl?: string;
  options: [QuizOption, QuizOption, QuizOption, QuizOption];
  correctOptionId: QuizOptionId;
  explanation: string;
  explanationAssisted?: string;
